...
export default function ShowTransportation({ navigation }) {
    ...
    useEffect(() => {
      ...
      function deletetransportation(id){
        fetch(`http://${host}:5000/delete_transportation/${id}`, {
            method: 'GET'
        })
        .then(res => res.json())
        .catch(error => console.log(error))
                 ...
                 <Card style={styles.card}>
                    ...
                    <Card.Content>
                        {(transportation && transportation.length) ?
                            <>
                                <Text style={styles.text2}>Transportation Charges: {transportation[0].transportation_charges}</Text>
                                <Text style={styles.text2}>Handling Charges: {transportation[0].handling_charges}</Text>
                                <Paragraph >
                                {Platform.OS=='android' ?
                            ...
                            <>
                            {Platform.OS=='android' ?
                                <Button mode="contained" style={{padding: '1%', marginTop: '2%'}} onPress={() => {navigation.navigate('AddTransportation')}}>Add Transportation</Button>
                                :
                                <Button mode="contained" style={{padding: '1%', marginTop: '2%'}}><Link to="/addTransportation">Add Transportation</Link></Button>
                            }
                            </>
                        }
                    </Card.Content>
